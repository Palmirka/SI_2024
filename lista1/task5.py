...
class Game:
    ...
    def column_score(self, x):
        column = self._get_column(x)
        score = Game.opt_dist({'fields': column, 'length': self.columns[x]})
        self.score_sum += score - self.column_dists[x]

        self.column_dists[x] = score
        self.update_bad(score, line_id=x + 1)

    def row_score(self, y: int):
        row = self._get_row(y)
        score = Game.opt_dist({'fields': row, 'length': self.rows[y]})
        self.score_sum += score - self.row_dists[y]

        self.row_dists[y] = score
        self.update_bad(score, line_id=-(y + 1))

    def update_bad(self, score, line_id):
    ...
    def check_score(self, x, y) -> int:
        ...
        column_score = Game.opt_dist({'fields': column, 'length': self.columns[x]})
        row_score = Game.opt_dist({'fields': row, 'length': self.rows[y]})
